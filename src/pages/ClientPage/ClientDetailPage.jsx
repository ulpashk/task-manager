...
export const ClientDetailPage = () => {
  ...
  if (!client) return <div className="p-10 text-center text-red-500">{t('common.no_data')}</div>;

  const stats = [
    { label: t('status.general'), value: client.task_summary?.total },
    { label: t('status.created'), value: client.task_summary?.created },
    { label: t('status.in_progress'), value: client.task_summary?.in_progress },
    { label: t('status.waiting'), value: client.task_summary?.waiting },
    { label: t('status.done'), value: client.task_summary?.done },
  ];

  const getJobTitle = (job) => {
    if (job === '') return t('common.no_data');
    if (job === 'manager') return t('role.manager');
    if (job === 'employee') return t('role.engineer');
    if (job === 'client') return t('role.client');
    if (job === 'engineer') return t('role.engineer');
    if (job === 'admin' || job === 'superadmin') return t('role.admin');
    return job;
  };
    ...
    <div className="flex flex-col gap-6 h-full overflow-hidden font-sans pb-4">
      <div className="relative bg-white p-8 rounded-2xl border border-gray-100 shadow-sm flex-shrink-0 flex items-center gap-8">
        ...
        <div className="flex-1 flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-gray-800 uppercase">{client.name}</h2>
            <span className="bg-[#FFFBE6] text-[#D4B363] border-[#FFE58F] border px-3 py-0.5 rounded text-xs font-medium">{client.client_type === 'company' ? t('clients.type_company') : client.client_type}</span>
          </div>
          <div className="text-[15px] text-gray-800 flex flex-col gap-1.5 mt-1">
            <p>{t('clients.email')}: <span className="text-gray-600">{client.email}</span></p>
            <p>{t('clients.phone')}: <span className="text-gray-600">{formatPhoneNumber(client.phone)}</span></p>
            <p>{t('clients.contact')}: <span className="text-gray-600">{client.contact_person}</span></p>
          </div>
        </div>
      </div>

      <div className="flex-shrink-0">
        <h3 className="text-sm font-bold text-gray-400 mb-3 uppercase tracking-wider">{t('clients.info')}</h3>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <div className="flex justify-between items-center mb-3">
           <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider">{t('users.all_users')}</h3>
           {isManager && (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col flex-1">
           <div className="p-4 border-b border-gray-100 flex justify-between gap-4">
              ...
              <div className="flex items-center gap-4 px-3 py-1.5 border border-gray-100 rounded-lg text-xs cursor-pointer hover:bg-gray-50 transition-colors">
                <span className="text-gray-600 font-medium">{t('common.sort_alpha')}</span>
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar">
             <table className="w-full text-left border-separate border-spacing-0">
               <thead className="bg-[#F9FAFB] text-[11px] font-bold text-gray-400 uppercase sticky top-0 z-10">
                 <tr>
                   <th className="px-8 py-3 border-b border-gray-100">{t('users.fio')}</th>
                   <th className="px-8 py-3 border-b border-gray-100">{t('users.email')}</th>
                   <th className="px-8 py-3 border-b border-gray-100 text-center">{t('users.role')}</th>
                   <th className="px-8 py-3 border-b border-gray-100">{t('users.phone')}</th>
                 </tr>
               </thead>
               <tbody className="divide-y divide-gray-50 text-[13px]">
                {client.employees.length !== 0 ? (
                  ...
                  <tr>
                    <td colSpan="4" className="px-8 py-6 text-center text-gray-400">{t('common.no_data')}</td>
                  </tr>
                )}
               </tbody>
             </table>
