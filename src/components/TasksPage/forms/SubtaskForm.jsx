  ...
  return (
    <form className="flex flex-col gap-5 font-sans" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[14px] font-bold text-gray-700">{t('tasks.subtask_form_title')}</label>
        <input
          required
          value={formData.title}
          onChange={(e) => setFormData({...formData, title: e.target.value})}
          className="bg-[#F9FAFB] border border-gray-200 rounded-lg px-4 py-3 outline-none focus:border-blue-500"
          placeholder={t('tasks.subtask_form_title_placeholder')}
        />
      </div>

      <div className="flex flex-col gap-1.5">
        <label className="text-[14px] font-bold text-gray-700">{t('tasks.subtask_form_description')}</label>
        <textarea
          required
          ...
          placeholder={t('tasks.subtask_form_description_placeholder')}
        />
      </div>

      {!isPredefined && (
        <div className="flex flex-col gap-1.5">
          <label className="text-[14px] font-bold text-gray-700">{t('tasks.wizard_task')}</label>
          <div className="relative">
            ...
            >
              <option value="" disabled>-- {t('tasks.wizard_task')} --</option>
              {parentTasks.map(tsk => (
      ...
      <MultiSelect
        label={t('tasks.subtask_form_assignee')}
        options={users}
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[14px] font-bold text-gray-700">{t('tasks.subtask_form_deadline')}</label>
        <input
          required
          type="datetime-local"
      ...
      <div className="flex justify-end gap-3 mt-6 border-t pt-6">
        <button type="button" onClick={onClose} className="px-8 py-2.5 font-bold text-gray-400 hover:text-gray-600 transition-colors">{t('common.cancel')}</button>
        <button
          type="submit"
          disabled={loading}
