  ...
  return (
    <div className="bg-white rounded-[20px] border border-gray-100 shadow-sm p-6 flex flex-col gap-4 font-sans h-full">
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`px-3 py-0.5 rounded-lg border text-[11px] font-bold ${statusStyles[epic.status] || statusStyles.created}`}>
            {(epic.status === 'done' || epic.status === 'completed')  ? t('epics.completed') : t('status.active')}
          </span>
          <ActionMenu 
      ...
      <div className="grid grid-cols-3 gap-2 text-[12px] mt-2">
        <div><span className="text-gray-400 block">{t('epics.manager')}:</span> <span className="font-bold text-gray-700">{epic.assignee?.first_name || t('epics.no_manager')} {epic.assignee?.last_name}</span></div>
        <div><span className="text-gray-400 block">{t('epics.total_tasks')}:</span> <span className="font-bold text-gray-700">{epic.tasks_count}</span></div>
        <div><span className="text-gray-400 block">{t('epics.progress')}:</span> <span className="font-bold text-gray-700">{getTaskProgress(epic.task_stats)}%</span></div>
      </div>

      <div className="flex flex-col gap-2 mt-2">
        <span className="text-[11px] font-bold text-gray-400 uppercase">{t('epics.team')}</span>
        <div className="flex flex-wrap gap-2">
          {epic.team && epic.team.length > 0 ? (
            ...
            <span className="text-gray-400 text-[10px]">{t('epics.no_manager')}</span>
          )}
      ...
      <div className="flex gap-4 text-[11px] font-bold border-t pt-4 mt-2">
        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-green-500"/> {t('epics.completed')}: {epic.task_stats?.completed || 0}</div>
        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-blue-500"/> {t('epics.in_progress')}: {epic.task_stats?.in_progress || 0}</div>
        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-gray-400"/> {t('epics.not_started')}: {epic.task_stats?.not_started || 0}</div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-4">
                <span className="text-[11px] text-gray-500 font-medium">{t('tasks.assignee')}:</span>
                <div className="flex flex-wrap gap-1.5">
                  {task.assignees && task.assignees.length > 0 ? (
                    ...
                    <span className="text-gray-400 text-[10px]">{t('tasks.no_assignees')}</span>
                  )}
                </div>
              </div>
              ...
              {hasSubtasks && (
                <div className="mt-4 space-y-2">
                  <p className="text-[11px] font-black text-gray-800 tracking-wider mb-2">{t('tasks.subtasks')}:</p>
